...
from cost_layers import SoftmaxLayer
...
class RecurrentLayerWithSearch(Layer):
    ...
    def _init_params(self):
        ...
        self.W_vc = theano.shared(
                sample_weights_classic(self.v_dim,
                    self.c_dim,
                    -1,
                    10 ** (-3),
                    rng=self.rng),
                name="V_%s"%self.name)
        self.params.append(self.W_vc)
        self.params_grad_scale = [self.grad_scale for x in self.params]
       
    def set_decoding_layers(self, c_inputer, c_reseter, c_updater, v_inputer, v_reseter, v_updater):
        self.c_inputer = c_inputer
        self.c_reseter = c_reseter
        self.c_updater = c_updater
        self.v_inputer = v_inputer
        self.v_reseter = v_reseter
        self.v_updater = v_updater
        for layer in [c_inputer, c_reseter, c_updater, v_inputer, v_reseter, v_updater]:
            self.params += layer.params
            self.params_grad_scale += layer.params_grad_scale

    # Step 6. the `v` related parameters is about the latent variable!
    def step_fprop(self,
                   state_below,
                   state_before,
                   gater_below=None,
        ...
        if not p_from_v:
            p_from_v = utils.dot(v, self.W_vc).reshape((source_num, self.c_dim))
        ...
        state_below += self.c_inputer(ctx).out + self.v_inputer(p_from_v).out
        reseter_below += self.c_reseter(ctx).out + self.v_reseter(p_from_v).out
        updater_below += self.c_updater(ctx).out + self.v_updater(p_from_v).out
    ...
    def fprop(self,
        ...
        updater_below = gater_below
        var_above = v
        assert v != None, 'Search Fprop V is None'
        ...
        if var_above.ndim > 1:
            p_from_v = utils.dot(var_above, self.W_vc).reshape(
                (var_above.shape[0], self.c_dim))
        else:
            p_from_v = utils.dot(var_above, self.W_vc).reshape(
                (1, self.c_dim))
        assert p_from_v != None, 'Search FProp p_from_v is None'
       
        if mask:
            sequences = [state_below, mask, updater_below, reseter_below]
            non_sequences = [c, c_mask, p_from_c, p_from_v] 
            #              seqs    | out |  non_seqs
            fn = lambda x, m, g, r,   h,   c1, cm, pc, pv : self.step_fprop(x, h, mask=m,
                    gater_below=g, reseter_below=r,
                    c=c1, p_from_c=pc, p_from_v=pv, c_mask=cm,
                    use_noise=use_noise, no_noise_bias=no_noise_bias,
                    return_alignment=return_alignment)
        else:
            sequences = [state_below, updater_below, reseter_below]
            non_sequences = [c, p_from_c, p_from_v]
            #            seqs   | out | non_seqs
            fn = lambda x, g, r,   h,    c1, pc, pv: self.step_fprop(x, h,
                    gater_below=g, reseter_below=r,
                    c=c1, p_from_c=pc, p_from_v=pv,
                    use_noise=use_noise, no_noise_bias=no_noise_bias,
                    return_alignment=return_alignment)
...
class EncoderDecoderBase(object):
    ...
    def _create_transition_layers(self):
        ...
        if rec_layer == RecurrentLayerWithSearch:
            add_args = dict(c_dim=self.state['c_dim'], v_dim=self.state['v_dim'])
        for level in range(self.num_levels):
...
class Decoder(EncoderDecoderBase):
    ...
    def create_layers(self):
        ...
        if self.state['search']:
            ...
            self.transitions[0].set_decoding_layers(
                    ...
                    self.var_reseters[0],
                    self.var_updaters[0])
            for layer in [self.variations[0], self.variation_mus[0], self.variation_sigmas[0], self.variations_post[0],\
                    self.variation_mus_post[0], self.variation_sigmas_post[0]]:
                self.transitions[0].params += layer.params
                self.transitions[0].params_grad_scale += layer.params_grad_scale

    def _create_variation_layers(self):
        logger.debug("_create_variation_layers")
        self.variations = [None] * self.num_levels
        self.variation_mus = [None] * self.num_levels
        self.variation_sigmas = [None] * self.num_levels
        self.variations_post = [None] * self.num_levels
        self.variation_mus_post = [None] * self.num_levels
        self.variation_sigmas_post = [None] * self.num_levels
        for level in range(self.num_levels):
            self.variations[level] = MultiLayer(
                self.rng,
                n_in=self.state['c_dim'],
                n_hids=self.state['v_dim'],
                activation=prefix_lookup(self.state, 'dec', 'activ'),
                bias_scale=self.state['bias'],
                name='{}_variation_{}'.format(self.prefix, level),
                **self.default_kwargs)
            #### Note that, dec active for variation_mus is linear
            self.variation_mus[level] = MultiLayer(
                self.rng,
                n_in=self.state['v_dim'],
                n_hids=self.state['v_dim'],
                activation="lambda x: x",
                bias_scale=self.state['bias'],
                name='{}_variation_mus_{}'.format(self.prefix, level),
                **self.default_kwargs)
            self.variation_sigmas[level] = MultiLayer(
                self.rng,
                n_in=self.state['v_dim'],
                n_hids=self.state['v_dim'],
                activation="lambda x: x",
                bias_scale=self.state['bias'],
                name='{}_variation_sigmas_{}'.format(self.prefix, level),
                **self.default_kwargs)
            self.variations_post[level] = MultiLayer(
                self.rng,
                n_in=self.state['c_dim']*2,
                n_hids=self.state['v_dim'],
                activation=prefix_lookup(self.state, 'dec', 'activ'),
                bias_scale=self.state['bias'],
                name='{}_variation_post_{}'.format(self.prefix, level),
                **self.default_kwargs)
            #### Note that, dec active for variation_mus is linear
            self.variation_mus_post[level] = MultiLayer(
                self.rng,
                n_in=self.state['v_dim'],
                n_hids=self.state['v_dim'],
                activation="lambda x: x",
                bias_scale=self.state['bias'],
                name='{}_variation_mus_post_{}'.format(self.prefix, level),
                **self.default_kwargs)
            self.variation_sigmas_post[level] = MultiLayer(
                self.rng,
                n_in=self.state['v_dim'],
                n_hids=self.state['v_dim'],
                activation="lambda x: x",
                bias_scale=self.state['bias'],
                name='{}_variation_sigmas_post_{}'.format(self.prefix, level),
                **self.default_kwargs)
    ...
    def _create_decoding_layers(self):
        ...
        self.var_inputers = [lambda x : 0] * self.num_levels
        self.var_reseters = [lambda x : 0] * self.num_levels
        self.var_updaters = [lambda x : 0] * self.num_levels
        ...
        if self.state['decoding_inputs']:
            for level in range(self.num_levels):
                # Input contributions
                self.var_inputers[level] = MultiLayer(
                    self.rng,
                    name='{}_var_inputter_{}'.format(self.prefix, level),
                    **decoding_kwargs)
                # Update gate contributions
                if prefix_lookup(self.state, 'dec', 'rec_gating'):
                    self.var_updaters[level] = MultiLayer(
                        self.rng,
                        name='{}_var_updater_{}'.format(self.prefix, level),
                        **decoding_kwargs)
                # Reset gate contributions
                if prefix_lookup(self.state, 'dec', 'rec_reseting'):
                    self.var_reseters[level] = MultiLayer(
                        self.rng,
                        name='{}_var_reseter_{}'.format(self.prefix, level),
                        **decoding_kwargs)
    ...
    def build_decoder(self, c, y,
        ...
        if mode == Decoder.EVALUATION:
            ...
            assert c_y
            assert c_y_mask
            self.transitions[0].params += c_y.params
            self.transitions[0].params_grad_scale += c_y.params_grad_scale
        else:
            assert not y_mask
            assert not c_y
            assert not c_y_mask
        ...
        # Step 1. obtain h_f from the source annotation vectors
        if c_mask:
            avg_ctx= TT.sum(c.out * c_mask[:, :, None], axis=0)
            avg_ctx = avg_ctx / TT.sum(c_mask, axis=0)[:, None]
            nsteps = avg_ctx.shape[0]
        else:
            avg_ctx = TT.mean(c, axis=0)
            if avg_ctx.ndim == 1:
                avg_ctx = avg_ctx.reshape((1, avg_ctx.shape[0]))
            nsteps = avg_ctx.shape[0]
        # Step 2. obtain h_e from the target annotation vectors, only during training
        if mode==Decoder.EVALUATION:
            if c_y_mask:
                avg_y_ctx= TT.sum(c_y.out * c_y_mask[:, :, None], axis=0)
                avg_y_ctx = avg_y_ctx / TT.sum(c_y_mask, axis=0)[:, None]
                nsteps_y = avg_y_ctx.shape[0]
            else:
                avg_y_ctx = TT.mean(c_y, axis=0)
                if avg_y_ctx.ndim == 1:
                    avg_y_ctx = avg_y_ctx.reshape((1, avg_y_ctx.shape[0]))
                nsteps_y = avg_y_ctx.shape[0]
        variation_layers = [] # the prior model
        if mode == Decoder.EVALUATION: 
            variation_post_layers = [] # the posterior model
        # Dimension: #sample x #variational_dimension
        for level in range(self.num_levels):
            # prior to h_z'
            variation_layers.append(self.variations[level](avg_ctx))
            if mode == Decoder.EVALUATION:
                # posterior to h_z'
                variation_post_layers.append(self.variations_post[level](Concatenate(axis=1)(*[avg_ctx, avg_y_ctx])))
        # Step 3. Gaussian Parameters w.r.t Prior and Posterior
        variation_mu_layers = []
        variation_log_sigma_layers = []
        variation_sigma_layers = []
        if mode == Decoder.EVALUATION:
            variation_mu_post_layers = []
            variation_log_sigma_post_layers = []
            variation_sigma_post_layers = []
        for level in range(self.num_levels):
            variation_mu_layers.append(self.variation_mus[level](variation_layers[level]))
            variation_log_sigma_layers.append(UnaryOp('lambda x: 0.5*x')(self.variation_sigmas[level](variation_layers[level])))
            variation_sigma_layers.append(UnaryOp('lambda x: TT.exp(x)')(variation_log_sigma_layers[level]))
            if mode == Decoder.EVALUATION:
                variation_mu_post_layers.append(self.variation_mus_post[level](variation_post_layers[level]))
                variation_log_sigma_post_layers.append(UnaryOp('lambda x: 0.5*x')(self.variation_sigmas_post[level](variation_post_layers[level])))
                variation_sigma_post_layers.append(UnaryOp('lambda x: TT.exp(x)')(variation_log_sigma_post_layers[level]))

        ### PRIOR
        # Step 4. Compute the KL objective
        self.train_cost = 0
        for level in range(self.num_levels):
            # prior = 0.5* T.sum(1 + 2*log_sigma_encoder - mu_encoder**2 - T.exp(2*log_sigma_encoder))
            # prior = 0.5 * TT.sum(1 + 2*variation_log_sigma_layers[level].out - variation_mu_layers[level].out**2 - TT.exp(2*variation_log_sigma_layers[level].out))
            # self.train_cost += -1 * prior
            if mode == Decoder.EVALUATION:
                logger.debug("KL divergency is added")
                # here, q(z|x,y) = post, p(z|x) = ~
                ...
                kl_der_q_p = (variation_log_sigma_layers[level] - variation_log_sigma_post_layers[level]) + \
                        (UnaryOp('lambda x: x**2')(variation_sigma_post_layers[level]) + \
                        UnaryOp('lambda x: x**2')(variation_mu_post_layers[level] - variation_mu_layers[level])) / (UnaryOp('lambda x: 2.*x**2')(variation_sigma_layers[level])) - 0.5
                self.train_cost = TT.sum(kl_der_q_p.out)

        # Generate variation hidden states
        def _gaussion_noise_step(mu, sigma, noise, z, add_noise=True):
            if not add_noise:
                return mu
            else:
                SIGMA = TT.diag(sigma)
                return mu + TT.dot(SIGMA, noise)

        # Step 5. Sample hz via hz = mu + sigma * eps
        sample_zs = []
        for level in range(self.num_levels):
            normal_noise = self.trng.normal(variation_mu_layers[level].shape)
            if mode != Decoder.EVALUATION:
                # during testing, we only use the prior
                seqs = [variation_mu_layers[level].out, variation_sigma_layers[level].out, normal_noise]
            else:
                # during training, we only use the posterior
                seqs = [variation_mu_post_layers[level].out, variation_sigma_post_layers[level].out, normal_noise]

            # noise is removed during testing
            sample_func = lambda m, s, n, z : _gaussion_noise_step(m, s, n, z, add_noise=False)
            if mode == Decoder.EVALUATION:
                # noise is important during training
                sample_func = lambda m, s, n, z : _gaussion_noise_step(m, s, n, z, add_noise=True)

            sample_z, _ = theano.scan(sample_func,
                    sequences=seqs,
                    outputs_info=[TT.alloc(0., self.state['v_dim'])],
                    name="variation_z_%s" % self.prefix,
                    n_steps = nsteps)
        ...
        for level in range(self.num_levels):
            ...
            if self.state['search']:
                ...
                # Step 6. injet the h_z into the decoder
                add_kwargs['v'] = sample_zs[level]
                if mode != Decoder.EVALUATION:
        ...
        if mode == Decoder.SAMPLING:
            ...
            # Step 7. add the KL-object as a regularizer
            return (self.output_layer.train(
                    state_below=readout,
                    target=y,
                    mask=y_mask,
                    reg=self.train_cost),   # added as a regularizer
                    alignment)
        else:
...
class RNNEncoderDecoder(object):
    ...
    def build(self):
        ...
        # Annotation for the log-likelihood computation
        """
        This part is the encoder for target sentence, we use the same bidirection-GRU as the source sentence.
        This encoder is only used for training, as a part of the posterior approximator.
        We tie the recurrent parameters of the source- and target-side encoder!
        """
        y_training_c_components = []

        """
        TODO:
        Maybe, the `backward_encoder.approx_embedder` could be replaced by the `decoder.approx_embedder`
        """
        logger.debug("Build y encoding computation graph")
        y_forward_training_c = self.encoder.build_encoder(
                self.y, self.y_mask,
                use_noise=True,
                approx_embeddings=self.backward_encoder.approx_embedder(self.y),
                return_hidden_layers=True)

        logger.debug("Build y backward encoding computation graph")
        y_backward_training_c = self.backward_encoder.build_encoder(
                self.y[::-1],
                self.y_mask[::-1],
                use_noise=True,
                approx_embeddings=self.backward_encoder.approx_embedder(self.y[::-1]),
                return_hidden_layers=True)
        # Reverse time for backward representations.
        y_backward_training_c.out = y_backward_training_c.out[::-1]

        if self.state['forward']:
            y_training_c_components.append(y_forward_training_c)
        if self.state['last_forward']:
            y_training_c_components.append(
                    ReplicateLayer(self.y.shape[0])(y_forward_training_c[-1]))
        if self.state['backward']:
            y_training_c_components.append(y_backward_training_c)
        if self.state['last_backward']:
            y_training_c_components.append(ReplicateLayer(self.y.shape[0])
                    (y_backward_training_c[0]))
        ...
        self.predictions, self.alignment = self.decoder.build_decoder(
                ...
                c_y=Concatenate(axis=2)(*y_training_c_components), c_y_mask=self.y_mask,
